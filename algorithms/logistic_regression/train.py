...
def process_dataset(dataset_path=None):
    ...
    X = df.drop(["Class", "Time"], axis=1)  # tried dropping the Time column
    y = df["Class"]
...
def train_simple(X_train, y_train):
    """
    Train a simple Logistic Regression model with default parameters.
...
def run(tuned=False, dataset_path=None, output_dir=None, create_cv_results=False, scoring="average_precision"):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="./models",
        help="Directory to save model artifacts. Defaults to ./data",
    )
